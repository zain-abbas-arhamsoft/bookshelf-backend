...
const { compressImage, uploadImagesToCloudinary } = require("../util/function");
exports.create = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: bookCreateSuccess, book });
  } catch (error) {
...
exports.deleteAllUserBooks = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "All user books deleted successfully" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Internal server error" });
  }
